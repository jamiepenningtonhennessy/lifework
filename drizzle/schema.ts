...
export const users = mysqlTable("users", {
  ...
  role: mysqlEnum("role", ["user", "counselor", "admin"]).default("user").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
...
// ─── Job Specification Reviews ──────────────────────────────────────────────
// Stores an uploaded job specification and the structured, evidence-led review
// generated by Alistair for a particular Lifework client.
export const jobSpecReviews = mysqlTable("job_spec_reviews", {
  id: int("id").autoincrement().primaryKey(),
  clientId: int("clientId").notNull(),
...
export type JobSpecReview = typeof jobSpecReviews.$inferSelect;
