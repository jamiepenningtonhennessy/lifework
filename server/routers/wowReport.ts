...
Keep the tone warm, direct, and non-judgmental. Begin with a brief framing sentence acknowledging this is a tool for self-awareness, not a fixed label. Write in second person ("You are...") throughout. Do NOT include any introductory paragraph before the first ## heading.`;
  ...
  const [
    ...
    callLLMWithTimeout(sys,
      `${ctx}\n\nWrite the Conclusions section for ${clientName}'s Lifework report. Structure it as follows:\n\n1. An opening paragraph (4-5 sentences) that introduces the overall pattern of ${clientName}'s dependable strengths — the thread that runs through life history, character strengths, and personality combined.\n\n2. One dedicated paragraph (4-5 sentences each) for EACH of ${clientName}'s top dependable strengths identified in this report. For each strength: name it clearly at the start of the paragraph, explain what the life history evidence shows about how this strength operates in ${clientName}'s life, connect it to the VIA and personality data, and say specifically what it means for ${clientName}'s career. Use a ## subheading for each strength paragraph (e.g. ## Creativity, ## Leadership).\n\n3. A closing paragraph (3-4 sentences) that is an encouraging, forward-looking statement about ${clientName}'s readiness and potential — speaking directly to ${clientName} as a wise mentor would.\n\nThis section should feel warm, specific, and genuinely personal. Do NOT include coaching questions or bullet points outside the strength paragraphs.`
    ),
  ]);
