...
Keep the tone warm, direct, and non-judgmental. Begin with a brief framing sentence acknowledging this is a tool for self-awareness, not a fixed label. Write in second person ("You are...") throughout. Do NOT include any introductory paragraph before the first ## heading.`;
  ...
  const [
    ...
    callLLMWithTimeout(sys,
      `${ctx}\n\nWrite 3-4 paragraphs analysing the recurring themes in ${clientName}'s life history, following this analytical principle: the earliest experiences carry the deepest imprint and establish the seed themes that reproduce — in different forms — throughout the rest of life. Think of it like a piece of music: the opening bars introduce the motifs that will recur, develop, and vary across all subsequent movements.\n\nStructure your analysis as follows:\n\n1. Begin with the EARLIEST achievements (childhood and adolescence). What themes, strengths, or patterns are already present here? These are the foundational motifs — the clearest signal of who ${clientName} truly is, before the world shaped ${subj} choices.\n\n2. Trace how those early themes REPRODUCE across the adult decades. Where do you see the same underlying pattern appearing in new contexts — different roles, different settings, but recognisably the same person? Reference specific achievements from the data.\n\n3. Note what the Enjoyable/Satisfying/Fulfilling classifications reveal about which activities connect to ${clientName}'s deepest motivational core — particularly where early-life E/S/F patterns reappear in adult life.\n\n4. Close with a synthesis: what is the single most consistent thread running from ${clientName}'s earliest experiences to ${subj} current professional identity? What does this tell us about where ${clientName} is most authentically ${subj}self?\n\nReference actual achievements from the data. Be specific about decades and ages where the data allows.`
    ),
    callLLMWithTimeout(sys,
    ...
    callLLMWithTimeout(sys,
      `${ctx}\n\nWrite the Conclusions section for ${clientName}'s Lifework report. Follow this analytical principle throughout: the earliest experiences carry the deepest imprint. The dependable strengths you identify should be rooted in what was already present in ${clientName}'s childhood and early life — not just what emerged in adulthood. These are the patterns that have been there from the beginning, expressing themselves in different forms across the decades.\n\nStructure it as follows:\n\n1. An opening paragraph (4-5 sentences) that names the core thread running through ${clientName}'s life from the earliest recorded experiences to today. This is the foundational motif — the theme that was already present before the world shaped ${clientName}'s choices, and that has reproduced itself in different forms ever since.\n\n2. One dedicated paragraph (4-5 sentences each) for EACH of ${clientName}'s top dependable strengths. For each strength: name it clearly at the start (use a ## subheading, e.g. ## Creativity), show when it FIRST appeared in the life history (the earlier the better), trace how it has recurred across the decades in different forms, connect it to the VIA and personality data, and say specifically what it means for ${clientName}'s career direction.\n\n3. A closing paragraph (3-4 sentences) that speaks directly to ${clientName} as a wise mentor would — acknowledging the journey from those early experiences to today, and expressing genuine confidence in what ${clientName} is ready to do next.\n\nThis section should feel warm, specific, and genuinely personal. Do NOT include coaching questions or bullet points outside the strength paragraphs.`
    ),
  ]);
