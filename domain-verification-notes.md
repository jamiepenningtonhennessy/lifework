...
- Follow-up verification after the hostname mapping change: `https://plumtrees-kfbbe6kq.manus.space/` now visibly renders the Lifework homepage. The management viewer screenshot showing Pennington Hennessy is therefore using a stale embedded preview and should be refreshed.
- Management-viewer diagnosis: the embedded Preview/Open Website view uses the live development hostname `3000-ivxro4qkiijpp7939byev-5763187f.us1.manus.computer`, not the managed project hostname. That development host is presently falling back to the Pennington Hennessy root route.
